...
Вы выделяете область экрана мышкой или включаете захват системного звука — приложение накладывает перевод поверх оригинала на экране или показывает субтитры на рабочем столе для аудио. Ничего не уходит в облако, если вы сами не включили облачный движок.

## Что отличает GhostLang

Большинство десктоп-переводчиков ограничиваются экранным OCR. Здесь экран и звук — равноправные. У них общий кеш переводов, общий глоссарий с защищёнными терминами, общий бенчмарк-harness. Переводите стрим — реплики и UI-tooltip'ы идут через один и тот же pipeline.

Ещё одна штука, которой нет у аналогов — **встроенный benchmark UI**: ablation-пресеты, разбивка latency по шагам, метрики CER/WER/BLEU/chrF, экспорт в CSV. Всё во вкладке Debug, без правки кода.
